...
<svelte:head>
    ...
    <meta
        ...
        content={product?.seo?.sitemapPriority
            ? product?.seo?.sitemapPriority
            : ""}
    />
    <meta
        name="sitemap_frequency"
        content={product?.seo?.sitemapFrequency
            ? product?.seo?.sitemapFrequency
            : "always"}
    />

    <meta
        property="og:image"
        content={product?.seo?.image ? host + product?.seo?.image : imageSeo}
    />
    <meta
        name="twitter:image"
        content={product?.seo?.image ? host + product?.seo?.image : imageSeo}
...
<h1
    class="my-10 text-primary-600 dark:text-white flex justify-center items-center"
>
    <Icon icon="ph:paw-print-bold" />{product.productName}<Icon
...
<div class="flex justify-center dark:text-slate-100 overflow-hidden">
    <div class="m-1 mb-10 container">
        <div class="w-full grid lg:grid-cols-3 grid-cols-1 gap-4">
            <div
                class="rounded-xl overflow-hidden md:col-span-2 {images.length <
                3
                    ? 'columns-' + images.length + ' w-full'
                    : images.length == 3
                    ? 'grid grid-cols-2 gap-4 '
                    : 'md:columns-3 columns-2 gap-4 '}"
            >
                {#each images as path, i}
                    <img
                        on:click={() => (imagesModel = true)}
                        src={!path
                            ? "/images/logo.png"
                            : `${host}` + "/" + path}
                        class="rounded-xl shadow-xl cursor-zoom-in {images.length <
                        3
                            ? 'w-full object-cover'
                            : images.length == 3
                            ? 'w-full'
                            : 'w-full object-contain mb-4'} {images.length ==
                            3 && i == 2
                            ? 'col-span-2'
                            : ''}"
                ...
                <div class="">
                    ...
                    <div class="mb-4">
                        <p class="text-right">
                            {#if product.discount}
                                <div class="">
                                    <b class="text-2xl text-primary-600"
                                        >{formatCurrency(product.price)}</b
                                    ><br /><br />
                                    <span
                                        class="p-2 animate-pulse rounded-full bg-red-600 text-white font-bold"
                                        >Discount {product.discount}%</span
                                    >
                                    <del
                                        class="text-gray-800 dark:text-gray-200"
                                        >{formatCurrency(product.price)}</del
                                    >
                                </div>
                            {:else}
                                <b class="text-2xl text-primary-600 mt-4 mb-10"
                                    >{formatCurrency(product.price)}</b
                                ><br />
                            {/if}
                        </p>
...
<Modal
    ...
    bind:open={imagesModel}
    autoclose
>
    <CarouselCustom perPage={1} classList="w-full max-h-screen">
        {#each images as path, i}
            <img
                src={!path ? "/images/logo.png" : `${host}` + "/" + path}
...
<div class="{isViewedImage ? '' : 'hidden'} absolute top-0 w-full h-full">
    <img
        src="https://demoda.vn/wp-content/uploads/2022/02/hinh-anh-dong-tinh-yeu-trai-tim-cam-roi.gif"
        alt="oke"
    />
</div>
