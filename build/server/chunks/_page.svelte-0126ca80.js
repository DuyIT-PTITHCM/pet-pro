...
import { c as create_ssr_component, v as validate_component, e as escape, b as each, a as add_attribute } from './ssr-e41dcf02.js';
...
import { T as Tabs, a as TabItem } from './CreateSeo-1d2324c7.js';
import { C as CreateProduct } from './CreateProduct-6e4397b7.js';
    ...
    {},
    {
      default: () => {
        return `${validate_component(TabItem, "TabItem").$$render($$result, { open: true, title: product?.productName }, {}, {
          default: () => {
            return `<div class="grid lg:grid-cols-2 grid-cols-1 gap-5 bg-white dark:bg-gray-800 rounded-b-lg"><div class="grid md:grid-cols-2 grid-cols-1 dark:text-gray-300"><div><p class="border-b p-2 m-4"><b>${escape($t("products.name"))}:</b> ${escape(product?.productName)}</p> <p class="border-b p-2 m-4"><b>${escape($t("products.type"))}:</b> ${escape(product?.type)}</p> <p class="border-b p-2 m-4"><b>${escape($t("products.status"))}:</b> ${escape(product?.status)}</p> <p class="border-b p-2 m-4"><b>${escape($t("common.category"))}:</b> ${escape(product?.category.categoryName)}</p> <p class="border-b p-2 m-4"><b>${escape($t("products.quantityInStock"))}:</b> ${escape(product?.stockQuantity)}</p> <p class="border-b p-2 m-4"><b>${escape($t("products.origin"))}:</b> ${escape(product?.origin)}</p> <p class="border-b p-2 m-4"><b>${escape($t("common.description"))}:</b> ${escape(product?.description)}</p></div> <div><p class="border-b p-2 m-4"><b>${escape($t("common.originalPrice"))}:</b> ${escape(formatCurrency(product?.originalPrice))}</p> <p class="border-b p-2 m-4"><b>${escape($t("common.price"))}:</b> ${escape(formatCurrency(product?.price))}</p> <p class="border-b p-2 m-4"><b>${escape($t("products.discount"))}:</b> ${escape(product?.discount)} %</p> ${product?.type != "pet" ? `<p class="border-b p-2 m-4"><b>${escape($t("products.expirationDate"))}:</b> ${escape(formatDate(product?.expirationDate))}</p>` : ``} <p class="border-b p-2 m-4"><b>${escape($t("products.notes"))} :</b> <!-- HTML_TAG_START -->${product?.notes}<!-- HTML_TAG_END --></p> <p class="border-b p-2 m-4"><b>${escape($t("common.slug"))}:</b> ${escape(product?.slug)}</p> <p class="border-b p-2 m-4"><b>${escape($t("common.createdAt"))} :</b> ${escape(formatDate(product?.createdAt))}</p> <p class="border-b p-2 m-4"><b>${escape($t("common.updatedAt"))} :</b> ${escape(formatDate(product?.updatedAt))}</p></div></div> <div class="leading-8 flex flex-col items-center"><b class="block dark:text-gray-300">${escape($t("common.images"))}</b> <div class="gap-4 md:columns-3 sm:columns-2 columns-1">${each(convertImageJsonToArray(product?.images), (path, i) => {
              ...
              {},
              {
                default: () => {
                  return `${validate_component(TabItem, "TabItem").$$render(
                    ...
                    {
                      open: true,
                      title: $t("seo.seoInformation")
                    },
                    {},
                    {
                      default: () => {
                        return `${product?.seo?.id ? `<div class="grid 2xl:grid-cols-2 xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 grid-cols-1 gap-5"><div class="text-gray-500 dark:text-gray-400"><p class="border-b p-2 m-4"><b>${escape($t("seo.metaTitle"))} : </b>${escape(product?.seo?.metaTitle)}</p> <p class="border-b p-2 m-4"><b>${escape($t("seo.metaDescription"))}</b> <br> ${escape(product?.seo?.metaDescription)}</p> <p class="border-b p-2 m-4"><b>${escape($t("seo.canonicalUrl"))} :</b> ${escape(product?.seo?.canonicalUrl)}</p> <p class="border-b p-2 m-4"><b>${escape($t("seo.keywords"))} :</b> ${escape(product?.seo?.keywords)}</p> <p class="border-b p-2 m-4"><b>${escape($t("seo.robotMetatags"))} :</b> ${escape(product?.seo?.robotMetaTags)}</p> <p class="border-b p-2 m-4"><b>${escape($t("seo.sitemapFrequency"))} :
                                    </b>${escape(product?.seo?.sitemapFrequency)}</p></div> <div>${validate_component(DeviceMockup, "DeviceMockup").$$render($$result, { device: "ios" }, {}, {
                          default: () => {
                        ...
                        })}</div></div>` : `${validate_component(Nodata, "Nodata").$$render($$result, {}, {}, {})}`}`;
                      }
                    }
                    ...
                    {
                      open: true,
                      title: $t("post.productPostDetail")
                    },
                    {},
                    {
                      default: () => {
                        return `${product?.post?.id ? `<div class="grid 2xl:grid-cols-2 xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-2 sm:grid-cols-1 grid-cols-1 gap-5"><div class="text-gray-500 dark:text-gray-300"><p class="border-b p-2 m-4">${escape($t("post.author"))} : ${escape(product?.post?.author)}</p> <p class="border-b p-2 m-4">${escape($t("common.category"))} : ${escape(product?.post?.category)}</p> <p class="border-b p-2 m-4">${escape($t("common.publishedAt"))} : ${escape(formatDate(product?.post?.published_at))}</p></div> <div><p class="py-[10px]">${escape($t("common.images"))}</p> <div class="grid grid-cols-3 gap-1">${each(convertImageJsonToArray(product?.post?.images), (path, i) => {
                          return `<img${add_attribute("src", !path ? "/images/logo.png" : `${HOST}/` + path, 0)} class="" alt="">`;
                        })}</div></div></div>  <div class="py-[20px] justify-center"><h3>${escape($t("post.content"))}</h3> <br> <div class="dark:text-gray-100"><!-- HTML_TAG_START -->${product?.post?.content}<!-- HTML_TAG_END --></div></div>` : `${validate_component(Nodata, "Nodata").$$render($$result, {}, {}, {})}`}`;
                      }
                    }
                  )}`;
                }
          ...
          default: () => {
            return `${validate_component(CreateProduct, "CreateProduct").$$render(
              $$result,
              {
                mode: MODE_MODIFY,
                products,
...
//# sourceMappingURL=_page.svelte-0126ca80.js.map
