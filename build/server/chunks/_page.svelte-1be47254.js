import { v as validate_store, a as subscribe, c as is_promise, n as noop } from './utils-ea986cb1.js';
import { c as create_ssr_component, e as escape, a as add_attribute, v as validate_component, b as each } from './ssr-ca1a55ab.js';
import { I as Icon } from './Indicator.svelte_svelte_type_style_lang-b4bd0601.js';
import { P as Products, C as CardPlaceholder } from './Products-33d9cac5.js';
...
import 'accounting';
import './CarouselCustom-65f35f2b.js';
import 'siema';
import './index-36a920ec.js';
import './cart-de007736.js';
...
const Blogs = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${blogs ? `${each(blogs, (blog, index) => {
    return `${function(__value) {
      ...
      return function(res) {
        return ` <div class="flex rounded-lg overflow-hidden border hover:opacity-80 hover:shadow-lg dark:text-white"><div class="w-[300px] bg-black overflow-hidden"><img${add_attribute("src", host + blogImages[0], 0)}${add_attribute("alt", blog.title, 0)} class="w-[300px] h-[250px] object-cover hover:scale-105 transition-all"></div> <div class="p-4 flex-1"><div class="flex flex-col justify-between h-full items-start"><h5>${escape(blog.title)}</h5> <p class="line-clamp-4 overflow-ellipsis text-justify relative" id="${"blog" + escape(index, true)}"><span>${escape(blog.description)}</span> ${showReadMore[index] ? `<a href="/" class="absolute bg-white dark:bg-slate-800 z-20 bottom-0 right-0">...<span class="text-blue-500" data-svelte-h="svelte-w3kk2g">Xem thêm</span></a>` : ``}</p> ${validate_component(Button, "Button").$$render($$result, { class: "px-10" }, {}, {
          default: () => {
            return `Xem thêm`;
          }
        })}</div> </div></div> `;
...
const css$1 = {
  code: ".service-box.s-OQm4IrrQMTE7 img.s-OQm4IrrQMTE7.s-OQm4IrrQMTE7{clip-path:circle(150% at 0 100%);margin:0;border-radius:0}.service-box.s-OQm4IrrQMTE7:hover img.s-OQm4IrrQMTE7.s-OQm4IrrQMTE7{clip-path:circle(50.0% at 0 100%);animation:clip-path .6s}@keyframes s-OQm4IrrQMTE7-hoverAnimate{from{clip-path:polygon(100% 0, 100% 100%, 25% 85%, 100% 100%, 0 100%, 0% 60%, 0 0)}to{clip-path:polygon(100% 0, 18% 25%, 20% 83%, 100% 100%, 0 100%, 0% 60%, 0 0)}}.service-box.s-OQm4IrrQMTE7:hover>div.s-OQm4IrrQMTE7>.service-desc.s-OQm4IrrQMTE7{display:block}",
  map: null
};
...
const Services = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${services ? `${each(services, (service, index) => {
    return `${function(__value) {
      ...
      return function(res) {
        return ` <div class="service-box rounded-lg overflow-hidden hover:shadow-lg hover:brightness-105 dark:text-white relative shadow-lg s-OQm4IrrQMTE7"><img${add_attribute("src", host + res[0], 0)}${add_attribute("alt", service.title, 0)} class="w-full h-[300px] object-cover transition-all s-OQm4IrrQMTE7"> <div class="xl:p-10 p-2 absolute z-10 top-0 w-full h-full flex justify-center items-center flex-col bg-black bg-opacity-30 text-white s-OQm4IrrQMTE7"><h5>${escape(service.title)}</h5> <div class="service-desc hidden s-OQm4IrrQMTE7"><p class="line-clamp-3 overflow-ellipsis text-justify relative m-4" id="${"service" + escape(index, true)}">${escape(service.description)} </p></div> ${validate_component(Button, "Button").$$render($$result, { href: "/", color: "primary" }, {}, {
          default: () => {
            return `Xem thêm`;
          }
        })} </div></div> `;
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${$$result.head += `<!-- HEAD_svelte-cl2f5j_START -->${$$result.title = `<title>${escape(data?.data?.seo?.metaTitle ? data?.data?.seo?.metaTitle : data?.data.name)}</title>`, ""}<meta name="description"${add_attribute(
    ...
    data?.data?.seo?.metaDescription ? data?.data?.seo?.metaDescription : data?.data.name,
    0
  )}><meta property="twitter:image"${add_attribute("content", HOST + data?.data?.seo?.image, 0)}><!-- HEAD_svelte-cl2f5j_END -->`, ""} <div class="flex relative gap-4 m-4 clear-both mb-10"><div class="sidebar-menu sticky top-0 left-0 uppercase w-[300px] dark:text-white lg:block hidden height-100vh border-r-4 s-_m32SN5m6RSJ"><div class="w-full rounded-lg text-sm"><ul class="list-none">${data?.data.parent_id ? `<p class="text-center p-4 text-shadow-xs flex items-center gap-1 justify-center text-lg">${validate_component(Icon, "Icon").$$render(
    $$result,
    {
    ...
    {},
    {}
  )}</h1> ${data?.data.isShowDescription == true && isShowDescription == true && data?.data.description ? `<h2 class="text-center text-2xl dark:text-white"><!-- HTML_TAG_START -->${data?.data.description}<!-- HTML_TAG_END --></h2>` : ``} <div class="flex justify-center items-center text-5xl"><hr class="w-20 h-1 bg-slate-600 dark:bg-white"> ${validate_component(Icon, "Icon").$$render(
    $$result,
    {
    ...
    return `<h3${add_attribute("id", category.id, 0)} class="dark:text-white uppercase text-xl font-bold flex items-center p-3">${escape(category.categoryName)}</h3> ${category.type == types[0] ? `<div class="grid 2xl:grid-cols-4 xl:grid-cols-3 lg:grid-cols-2 sm:grid-cols-2 grid-cols-1 gap-4">${validate_component(Products, "Products").$$render($$result, { products: category.products }, {}, {})} </div>` : `${category.type == types[1] ? `<div class="grid xl:grid-cols-2 grid-cols-1 gap-4">${validate_component(Blogs, "Blogs").$$render($$result, { blogs: category.posts }, {}, {})} </div>` : `${category.type == types[2] ? `<div class="grid xl:grid-cols-3 lg:grid-cols-2 grid-cols-1 gap-4">${validate_component(Services, "Services").$$render($$result, { services: category.posts }, {}, {})} </div>` : ``}`}`}`;
  })}</div>` : `${data?.data.isShowDescription == true && isShowDescription == true && data?.data.description ? `<h1 class="text-center text-3xl my-10 dark:text-white"><!-- HTML_TAG_START -->${data?.data.description}<!-- HTML_TAG_END --></h1>` : ``} ${each(data?.data?.subMenus, (submenu) => {
    return `<div class="w-full dark:text-white mt-5"><h2 class="uppercase text-3xl font-bold flex justify-center items-center p-3">${validate_component(Icon, "Icon").$$render(
      ...
      return `<h3${add_attribute("id", category.id, 0)} class="dark:text-white uppercase text-xl font-bold flex items-center p-3">${escape(category.categoryName)}</h3> ${category.type == types[0] ? `<div class="grid 2xl:grid-cols-4 xl:grid-cols-3 lg:grid-cols-2 sm:grid-cols-2 grid-cols-1 gap-4">${validate_component(Products, "Products").$$render($$result, { products: category.products }, {}, {})} </div>` : `${category.type == types[1] ? `<div class="grid 2xl:grid-cols-2 grid-cols-1 gap-4">${validate_component(Blogs, "Blogs").$$render($$result, { blogs: category.posts }, {}, {})} </div>` : `${category.type == types[2] ? `<div class="grid xl:grid-cols-4 lg:grid-cols-2 grid-cols-1 gap-4">${validate_component(Services, "Services").$$render($$result, { services: category.posts }, {}, {})} </div>` : ``}`}`}`;
    })} </div>`;
  })}`}</div></div> </div>`;
});

export { Page as default };
//# sourceMappingURL=_page.svelte-1be47254.js.map
