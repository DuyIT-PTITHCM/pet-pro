...
const css$1 = {
  code: ".service-box.s-OQm4IrrQMTE7:hover img.s-OQm4IrrQMTE7.s-OQm4IrrQMTE7{opacity:.9;border-top-right-radius:0;border-bottom-right-radius:0;margin-left:0}.service-box.s-OQm4IrrQMTE7:hover>div.s-OQm4IrrQMTE7>.service-desc.s-OQm4IrrQMTE7{display:block;font-weight:500;line-clamp:4}",
  map: null
};
...
const Services = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${services ? `${each(services, (service, index) => {
    return `${function(__value) {
      ...
      return function(res) {
        return ` <a href="/" class="service-box rounded-lg overflow-hidden hover:shadow-lg hover:brightness-105 dark:text-white relative shadow-lg s-OQm4IrrQMTE7"><img${add_attribute("src", host + blogImages[0], 0)}${add_attribute("alt", service.title, 0)} class="w-full h-[400px] rounded-full ml-[-40%] overflow-hidden object-cover transition-all s-OQm4IrrQMTE7"> <div class="xl:p-10 p-2 absolute z-10 top-0 w-full h-full flex justify-center items-end flex-col hover:bg-white hover:bg-opacity-50 s-OQm4IrrQMTE7"><h5>${escape(service.title)}</h5> <div class="service-desc hidden s-OQm4IrrQMTE7"><p class="line-clamp-4 overflow-ellipsis text-justify relative m-4 lg:text-[18px] text-base" id="${"service" + escape(index, true)}">${escape(service.description)} </p></div> ${validate_component(Button, "Button").$$render($$result, { href: "/", color: "primary" }, {}, {
          default: () => {
            return `Xem thêm`;
          }
        })} </div></a> `;
      }();
    }(getImage(service.imageUrl))}`;
  })}` : ``}`;
});
const css = {
...
//# sourceMappingURL=_page.svelte-38b05c7d.js.map
