import { v as validate_store, a as subscribe, j as is_promise, n as noop } from './utils-22ec5563.js';
...
import { C as CardPlaceholder } from './CardPlaceholder-368d9226.js';
import { H as HeaderPage } from './HeaderPage-625b8d47.js';
import { s as sineIn } from './index4-e6d78c98.js';
import { C as CloseButton } from './CloseButton-5ce65cd1.js';
import { D as Drawer } from './Drawer-daf14135.js';
import './accounting-d72f8257.js';
import 'accounting';
import './CarouselCustom-5ab97610.js';
import 'siema';
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    $$settled = true;
    $$rendered = `${$$result.head += `<!-- HEAD_svelte-cl2f5j_START -->${$$result.title = `<title>${escape(data?.data?.seo?.metaTitle ? data?.data?.seo?.metaTitle : data?.data.name)}</title>`, ""}<meta name="description"${add_attribute(
      "content",
      ...
      data?.data?.seo?.metaDescription ? data?.data?.seo?.metaDescription : data?.data.name,
      0
    )}><meta property="twitter:image"${add_attribute("content", HOST + data?.data?.seo?.image, 0)}><!-- HEAD_svelte-cl2f5j_END -->`, ""} ${data?.data.isShowDescription == true && isShowDescription == true && data?.data.description ? `${validate_component(HeaderPage, "HeaderPage").$$render(
      $$result,
      {
...
//# sourceMappingURL=_page.svelte-c4397cd3.js.map
