...
const PostProductDetail = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.post === void 0 && $$bindings.post && post !== void 0)
    $$bindings.post(post);
  return `<div class="md:grid md:grid-cols-10 gap-4 dark:text-slate-50"><div class="p-5 md:col-span-2 border rounded-xl"><div class=""><h3 class="uppercase">${escape(post?.title)}</h3> <p>Author: ${escape(post?.author)}</p> <p>Date: ${escape(post?.updatedAt)}</p></div> <div class="w-full h-9 bg-red-600 sticky top-[100px]"></div></div> <div id="postcontent" class="post-container overflow-hidden md:col-span-8"><!-- HTML_TAG_START -->${post?.content}<!-- HTML_TAG_END --></div> </div>`;
});
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `${$$result.head += `<!-- HEAD_svelte-1l85s6z_START -->${$$result.title = `<title>${escape(product?.seo?.metaTitle ? product?.seo?.metaTitle : product.productName)}</title>`, ""}<meta name="description"${add_attribute(
      ...
      return `<img${add_attribute("src", !path ? "/images/logo.png" : `${host}/` + path, 0)} class="${"rounded-xl cursor-zoom-in " + escape(
    ...
    })}</div> <div class="w-full h-full"><div class="rounded-lg p-4 border h-full flex flex-col justify-between"><div><h2 class="mb-4 text-xl font-bold">${escape(product.productName)}</h2> ${validate_component(Hr, "Hr").$$render($$result, { classHr: "my-8 w-64 h-1", icon: true }, {}, {
      default: () => {
    ...
    })}  <p class="mb-4"><!-- HTML_TAG_START -->${product.description}<!-- HTML_TAG_END --></p> ${validate_component(Hr, "Hr").$$render($$result, { classHr: "my-8 w-64 h-1", icon: true }, {}, {
      default: () => {
        return `${validate_component(Icon, "Icon").$$render(
          $$result,
          {
    ...
    })} <div class="mb-4 product-desc" id="note"><!-- HTML_TAG_START -->${product.notes}<!-- HTML_TAG_END --></div></div> <div>${validate_component(Hr, "Hr").$$render($$result, { classHr: "my-8 w-64 h-1", icon: true }, {}, {
      default: () => {
        return `${validate_component(Icon, "Icon").$$render(
          $$result,
          {
    ...
    })} <div class="mb-4"><p class="text-right">${product.discount ? `<div class=""><b class="text-2xl text-primary-600">${escape(formatCurrency(product.price))}</b><br><br> <div class="flex justify-between items-center"><span class="p-2 animate-pulse rounded-full bg-red-600 text-white font-bold">Discount ${escape(product.discount)}%</span> <del class="text-gray-800 dark:text-gray-200">${escape(formatCurrency(product.price))}</del></div></div>` : `<b class="text-2xl text-primary-600 mt-4 mb-10">${escape(formatCurrency(product.price))}</b><br>`}</p></div> <div class="flex justify-between w-full mt-[40px]">${validate_component(Button, "Button").$$render($$result, { class: "px-4" }, {}, {
      default: () => {
        return `${validate_component(Icon, "Icon").$$render(
          $$result,
          {
    ...
    })}</div></div></div></div></div></div></div> ${validate_component(Modal, "Modal").$$render(
      $$result,
      {
        size: "xl",
        class: "md:h-screen",
...
//# sourceMappingURL=_page.svelte-53daa555.js.map
