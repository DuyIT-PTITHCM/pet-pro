...
import { b as Textarea, T as Tabs, a as TabItem, C as CreateSeo } from './CreateSeo-12646cbc.js';
import moment from 'moment';
import 'js-cookie';
import { E as Editor_1 } from './Editor-03d75bad.js';
...
const CreateProduct = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    $$settled = true;
    $$rendered = `<div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 bg-white dark:bg-slate-800 dark:text-white rounded p-4 h-full"><h1 class="text-[34px] py-[10px] uppercase text-center font-bold col-span-full">${escape(title)}</h1>  <div class="px-3"><label class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2 text-[14px] py-[10px]" for="grid-first-name" data-svelte-h="svelte-1jspw1">Product Name</label> <input class="appearance-none dark:bg-gray-700 block w-full bg-grey-lighter text-grey-darker border border-red rounded py-3 px-4 mb-3" id="grid-first-name" type="text" placeholder="Cat Indo"${add_attribute("value", products.productName, 0)}></div> <div class="px-3"><label class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2 text-[14px] py-[10px]" for="grid-last-name" data-svelte-h="svelte-3i17ou">Product Description</label> ${validate_component(Textarea, "Textarea").$$render(
      $$result,
      {
      ...
      {}
    )}</div></div> <div class="col-span-full p-3"><label class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2 text-[14px] py-[10px]" for="grid-zip" data-svelte-h="svelte-z9xrug">Images Products</label> ${validate_component(Fileupload, "Fileupload").$$render($$result, { class: "w-24 py-[10px] bg-white" }, {}, {})} <div class="grid grid-cols-1 xl:grid-cols-4 gap-[10px] py-[20px]">${each(files, (path) => {
...
//# sourceMappingURL=CreateProduct-06d46256.js.map
