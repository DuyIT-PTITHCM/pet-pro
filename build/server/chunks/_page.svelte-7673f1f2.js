...
import { P as Products, C as CardPlaceholder } from './Products-dac413e1.js';
...
const Blogs = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${blogs ? `${each(blogs, (blog, index) => {
    return `${function(__value) {
      ...
      return function(res) {
        return ` <a href="/" class="flex rounded-lg overflow-hidden border hover:shadow-lg dark:text-white max-h-[140px]"><div class="overflow-hidden" style="${"width: " + escape(serviceBoxWidth * 30 / 100, true) + "px;"}"><img${add_attribute("src", host + blogImages[0], 0)}${add_attribute("alt", blog.title, 0)} class="w-full h-full object-cover hover:scale-105 transition-all"></div> <div class="md:p-4 p-3 flex-1"><div class="flex flex-col justify-between items-start"><h3 class="md:text-lg text-base font-semibold line-clamp-2">${escape(blog.title)}</h3> <p class="md:text-base text-sm mt-1 line-clamp-2 text-justify relative" id="${"blog" + escape(index, true)}"><span>${escape(blog.description)}</span> ${showReadMore[index] ? `<a href="/" class="absolute bg-white dark:bg-slate-800 z-20 bottom-0 right-0">...<span class="text-blue-500" data-svelte-h="svelte-w3kk2g">Xem thêm</span></a>` : ``} </p></div> </div></a> `;
      }();
...
//# sourceMappingURL=_page.svelte-7673f1f2.js.map
